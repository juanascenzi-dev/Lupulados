...
import { getActivePromotion } from "@/domain/commercialSelectors";
...
export function SharedCheckoutPanel({
  ...
  const { items, updateQty, removeItem, clearCart, orderSummary, extras, setExtras, totalItems } =
    useCart();
  ...
  const deliveryOptions = snapshot.deliveryOptions
  ...
  const promotion = getActivePromotion(snapshot);
  ...
  const applyPromo = () => {
    ...
    setExtras((prev) => ({ ...prev, promoCode: "", discount: 0, discountType: "percentage" }));
  ...
  try {
    whatsAppUrl =
      selectedChannel && validation.valid
        ? buildWhatsAppOrderUrl(selectedChannel.phoneE164, message)
        : null;
  } catch {
    whatsAppUrl = null;
  }
    ...
    <div className={cn("grid gap-5", compact ? "" : "lg:grid-cols-[minmax(0,1fr)_360px]")}>
      <div className="space-y-5">
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          <div className="flex items-center justify-between gap-3 mb-3">
            ...
            {items.length > 0 && (
              <button
                type="button"
                onClick={clearCart}
                className="text-xs font-bold text-white/45 hover:text-red-300 flex items-center gap-1"
              >
          ...
          {items.length === 0 ? (
            <p
              className="rounded-xl border border-dashed border-white/10 bg-black/20 p-4 text-sm text-white/55"
              role="status"
            >
                ...
                >
                    ...
                    <div className="min-w-0">
                      ...
                      {item.pack?.type === "configurable-beer-pack" && (
                        <p className="mt-1 text-xs text-white/45">
                          Precio por pack: {formatPrice(item.price)}
                        </p>
                      )}
                      {item.promotional && (
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-white/45 hover:text-red-300"
                    >
                      Eliminar
                    </button>
                  </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-white/65">
                Nombre *
              </span>
              <input
                value={formData.name}
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  setFormData((prev) => ({ ...prev, eventDate: event.target.value }))
                }
                className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white [color-scheme:dark] focus:border-primary focus:outline-none"
              />
            </label>
            <label className="block">
              <span className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-white/65">
          ...
          <label className="mt-4 block">
            ...
            <textarea
              ...
              className="h-24 w-full resize-none rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white focus:border-primary focus:outline-none"
            />
          </label>
        </section>
      </div>
      ...
      <aside className="space-y-4">
        <section className="rounded-2xl border border-primary/20 bg-primary/10 p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between gap-3">
              <span className="text-white/55">Productos</span>
              <span className="font-mono text-white">
                {formatPrice(orderSummary.itemsSubtotal)}
              </span>
            </div>
            {orderSummary.extrasTotal > 0 && (
              <div className="flex justify-between gap-3">
                <span className="text-white/55">Extras</span>
                <span className="font-mono text-white">
                  {formatPrice(orderSummary.extrasTotal)}
                </span>
              </div>
            )}
            <div className="flex justify-between gap-3">
              <span className="text-white/55">Envio</span>
              <span className="font-mono text-white">{formatPrice(orderSummary.deliveryCost)}</span>
            </div>
            {orderSummary.discountAmount > 0 && (
              <div className="flex justify-between gap-3 text-green-300">
                <span>Promo {orderSummary.discountCode}</span>
                <span className="font-mono">-{formatPrice(orderSummary.discountAmount)}</span>
              </div>
            )}
            <div className="border-t border-white/10 pt-3 flex justify-between gap-3 text-lg font-black">
              <span className="text-white">Total estimado</span>
              <span className="font-mono text-primary">{formatPrice(orderSummary.total)}</span>
            </div>
          </div>
          {demoNotice && (
            <p className="mt-3 rounded-xl border border-amber-400/20 bg-black/20 p-3 text-xs leading-relaxed text-amber-100">
              Catálogo de demostración. Productos, precios y disponibilidad son ilustrativos y se
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 space-y-4">
          <div>
            <label
              htmlFor="shared-promo-code"
              className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-white/65"
            >
              Promoción
            </label>
            <div className="flex gap-2">
              <input
                id="shared-promo-code"
                value={promoInput}
                onChange={(event) => setPromoInput(event.target.value)}
                placeholder={promotion?.code ? `Ej: ${promotion.code}` : "Codigo"}
                className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-white placeholder:text-white/30 focus:border-primary focus:outline-none"
              />
              <button
                type="button"
                onClick={applyPromo}
                className="rounded-xl border border-white/10 bg-white/10 px-4 text-sm font-bold text-white hover:bg-white/15"
              >
                Aplicar
              </button>
            </div>
            {promoStatus === "valid" && (
              <p className="mt-2 text-xs font-bold text-green-300" role="status">
                Promoción aplicada.
              </p>
            )}
            {promoStatus === "invalid" && (
          ...
          {!validation.valid && (
            <div
              className="rounded-xl border border-red-400/20 bg-red-500/10 p-3 text-xs text-red-100"
              role="alert"
            >
              {validation.errors[0]}
            </div>
          ...
          {whatsAppUrl ? (
            <a
              ...
              className={cn(
                "flex w-full items-center justify-center gap-2 rounded-2xl bg-[#25D366] px-4 py-3.5 text-center font-bold text-white transition-colors hover:bg-[#1db954]",
                opening ? "opacity-70" : "",
              )}
            >
            ...
            <button
              type="button"
              disabled
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white/10 px-4 py-3.5 text-center font-bold text-white/35"
            >
              Preparar WhatsApp
            </button>
          )}
