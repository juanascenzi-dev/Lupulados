...
describe("admin form adapters", () => {
  ...
  it("preloads and converts presentation prices as non-negative numbers", () => {
    ...
    expect(presentationToFormValues(presentation)).toMatchObject({ id: presentation.id, unitPrice: presentation.unitPrice });
    expect(parsePresentationUpdateForm({
    ...
    }, commercialSnapshot.products)).toMatchObject({ unitPrice: 0, description: null });
    expect(() => parsePresentationUpdateForm({
      productId: presentation.productId,
      presentationType: presentation.presentationType,
      label: presentation.label,
      ...
      description: "",
      sortOrder: String(presentation.sortOrder),
    }, commercialSnapshot.products)).toThrow();
  });
  ...
  it("validates that edited presentations reference an existing product", () => {
    ...
    expect(() => parsePresentationUpdateForm({
      ...
      description: "",
      sortOrder: String(presentation.sortOrder),
    }, commercialSnapshot.products)).toThrow("El producto relacionado no existe.");
  });
  ...
  it("updates delivery and extra form values with numeric prices", () => {
    ...
    expect(deliveryToFormValues(delivery)).toMatchObject({ id: delivery.id, requiresAddress: delivery.requiresAddress });
    expect(extraToFormValues(extra)).toMatchObject({ id: extra.id, unit: extra.unit });
    expect(parseDeliveryUpdateForm({
      label: "Retiro",
  ...
  it("normalizes promotion codes, validates percentages, and persists empty dates as null", () => {
    ...
    expect(() => parsePromotionUpdateForm({
      code: "alto",
      type: "percentage",
      value: "1.5",
      startDate: "",
