...
export const productSchema = z.object({
  ...
  mainCategory: z.enum(["beer", "alcohol", "non-alcohol", "combo", "accessory"]).optional(),
  subcategory: z.string().trim().min(1).optional(),
  tags: z.array(z.string().trim().min(1)).optional(),
  demo: z.boolean().optional(),
  demoNote: z.string().trim().min(1).optional(),
  ...
  image: z.string().url().or(z.string().regex(/^\/[^\s]+$/)).or(z.literal("")),
...
export const productPresentationSchema = z.object({
  id: idSchema,
  productId: idSchema,
  presentationType: idSchema,
  label: z.string().trim().min(1),
  volumeLiters: positiveVolumeSchema,
  unitPrice: nonNegativePriceSchema,
  category: z.enum(["barril", "growler", "porrón", "pack"]),
  comparisonGroup: z.string().trim().min(1).optional(),
  comparisonQuantity: positiveVolumeSchema.optional(),
  comparisonUnit: z.string().trim().min(1).optional(),
  unitsPerPresentation: positiveVolumeSchema.optional(),
  compareAtPrice: nonNegativePriceSchema.optional(),
  promotional: z.boolean().optional(),
  promotionLabel: z.string().trim().min(1).optional(),
