...
export default function AdminDashboard() {
  ...
  const loadAdminData = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "No se pudieron cargar los datos administrativos", description: getFirstZodError(error, "Error inesperado.") });
    } finally {
  ...
  const loadAudit = async () => {
    ...
    try {
      ...
      toast({ title: "No se pudo cargar la actividad", description: getFirstZodError(error, "Error inesperado.") });
    } finally {
  ...
  const presentations = adminData.presentations.filter((item) => matchesStatus(item.active, presentationStatus));
  const deliveryOptions = adminData.deliveryOptions.filter((item) => matchesStatus(item.active, deliveryStatus));
  const extraOptions = adminData.extraOptions.filter((item) => matchesStatus(item.active, extraStatus));
  const promotions = adminData.promotions.filter((item) => matchesStatus(item.active, promotionStatus));
  ...
  return (
    <main className="min-h-screen bg-background text-white">
      <header className="border-b border-white/10 bg-card">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" disabled={busy || adminLoading} onClick={() => void refreshEverything()}>
              <RefreshCw className="w-4 h-4 mr-2" /> Refrescar
            </Button>
            <Button variant="outline" disabled={busy} onClick={() => void auth.signOut().then(() => navigate("/admin/login", { replace: true }))}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {adminLoading && <p className="mb-4 text-sm text-muted-foreground">Cargando datos administrativos...</p>}
        <Tabs defaultValue="summary">
          ...
          <TabsContent value="summary">
            <MetricGrid
              metrics={[
                ["Productos activos", adminData.products.filter((product) => product.status === "active").length],
                ["Productos archivados", adminData.products.filter((product) => product.status === "archived").length],
                ["Presentaciones activas", adminData.presentations.filter((item) => item.active).length],
                ["WhatsApp activos", adminData.whatsappChannels.filter((item) => item.active).length],
          ...
          <TabsContent value="products" className="space-y-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre o ID" className="max-w-sm bg-card border-white/10" />
            ...
            <ProductForm
              ...
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingProduct ? repository!.updateProduct(editingProduct.id, parseProductUpdateForm(input)) : repository!.createProduct(parseProductForm(input)),
          ...
          <TabsContent value="presentations" className="space-y-4">
            ...
            <PresentationForm
              ...
              disabled={busy}
              initial={editingPresentation ? presentationToFormValues(editingPresentation) : undefined}
              onCancel={editingPresentation ? () => setEditingPresentation(null) : undefined}
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingPresentation
                  ? repository!.updatePresentation(editingPresentation.id, parsePresentationUpdateForm(input, adminData.products))
                  : repository!.createPresentation(parsePresentationForm(input, adminData.products)),
          ...
          <TabsContent value="whatsapp" className="space-y-4">
            ...
            <WhatsAppForm
              ...
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingWhatsApp ? repository!.updateWhatsAppChannel(editingWhatsApp.id, parseWhatsAppUpdateForm(input)) : repository!.createWhatsAppChannel(parseWhatsAppForm(input)),
            ...
            <AdminTable
              ...
              render={(channel) => [
                ...
                <div key="actions" className="flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" disabled={busy || !channel.active} onClick={() => run(() => repository!.setPrimaryWhatsAppChannel(channel.id), "WhatsApp principal actualizado")}>Principal</Button>
                  <RowActions
                    archived={!channel.active}
                    disabled={busy}
                    onEdit={() => setEditingWhatsApp(channel)}
                    onArchive={() => confirmArchive(channel.label) && void run(() => repository!.archiveWhatsAppChannel(channel.id), "WhatsApp desactivado")}
                    onRestore={() => run(() => repository!.updateWhatsAppChannel(channel.id, { active: true }), "WhatsApp restaurado")}
                  />
                </div>,
              ]}
            />
          ...
          <TabsContent value="delivery" className="space-y-4">
            ...
            <DeliveryForm
              ...
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingDelivery ? repository!.updateDeliveryOption(editingDelivery.id, parseDeliveryUpdateForm(input)) : repository!.createDeliveryOption(parseDeliveryForm(input)),
            ...
            <ToggleTable
              ...
              onArchive={(row) => confirmArchive(row.label) && void run(() => repository!.archiveDeliveryOption(row.id), "Entrega desactivada")}
              onRestore={(row) => run(() => repository!.restoreDeliveryOption(row.id), "Entrega restaurada")}
            />
          </TabsContent>
          ...
          <TabsContent value="extras" className="space-y-4">
            ...
            <ExtraForm
              ...
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingExtra ? repository!.updateExtraOption(editingExtra.id, parseExtraUpdateForm(input)) : repository!.createExtraOption(parseExtraForm(input)),
            ...
            <ToggleTable
              ...
              onArchive={(row) => confirmArchive(row.label) && void run(() => repository!.archiveExtraOption(row.id), "Extra desactivado")}
              onRestore={(row) => run(() => repository!.restoreExtraOption(row.id), "Extra restaurado")}
            />
          </TabsContent>
          ...
          <TabsContent value="promotions" className="space-y-4">
            ...
            <PromotionForm
              ...
              onSubmit={(input, mode) => run(
                () => mode === "edit" && editingPromotion ? repository!.updatePromotion(editingPromotion.id, parsePromotionUpdateForm(input)) : repository!.createPromotion(parsePromotionForm(input)),
            ...
            <AdminTable
              ...
              render={(promotion) => [
                ...
                <RowActions
                  ...
                  onArchive={() => confirmArchive(promotion.code) && void run(() => repository!.archivePromotion(promotion.id), "Promoción desactivada")}
                  onRestore={() => run(() => repository!.restorePromotion(promotion.id), "Promoción restaurada")}
                />,
              ]}
            />
          </TabsContent>
          ...
          <TabsContent value="activity" className="space-y-4">
            <Button variant="outline" disabled={auditLoading} onClick={() => void loadAudit()}>
              <RefreshCw className="w-4 h-4 mr-2" /> {auditLoading ? "Cargando..." : "Cargar actividad reciente"}
            </Button>
            <AdminTable
              rows={audit}
              columns={["Fecha", "Tabla", "Operación", "Registro"]}
              render={(entry) => [formatArgentinaDate(entry.createdAt), entry.tableName, entry.operation, entry.recordId ?? "-"]}
            />
          </TabsContent>
        </Tabs>
      </div>
...
function RowActions({ archived, disabled, onEdit, onArchive, onRestore }: { archived: boolean; disabled: boolean; onEdit: () => void; onArchive: () => void; onRestore: () => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      <Button size="sm" variant="outline" disabled={disabled} onClick={onEdit}><Edit className="w-4 h-4 mr-1" /> Editar</Button>
      {archived ? (
...
function ProductForm({ disabled, initial, onCancel, onSubmit }: { disabled: boolean; initial?: ProductFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar producto ${editId}` : "Crear producto"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear producto"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parseProductUpdateForm(input) : parseProductForm(input);
        setError("");
      } catch (error) {
      ...
      <Field name="description" label="Descripción" defaultValue={initial?.description} className="md:col-span-2" />
      <Field name="image" label="URL imagen" defaultValue={initial?.image} className="md:col-span-2" />
      <SelectField name="category" label="Categoría" options={["beer", "pack"]} defaultValue={initial?.category ?? "beer"} />
      <Field name="sortOrder" label="Orden" type="number" defaultValue={initial?.sortOrder ?? 0} />
      <Field name="abv" label="ABV" type="number" step="0.1" defaultValue={initial?.abv ?? ""} required={false} />
      <Field name="ibu" label="IBU" type="number" step="1" defaultValue={initial?.ibu ?? ""} required={false} />
      <Field name="badge" label="Badge" defaultValue={initial?.badge ?? ""} required={false} />
    </AdminForm>
  );
}

function PresentationForm({ products, disabled, initial, onCancel, onSubmit }: { products: Product[]; disabled: boolean; initial?: PresentationFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar presentación ${editId}` : "Crear presentación"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear presentación"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parsePresentationUpdateForm(input, products) : parsePresentationForm(input, products);
        setError("");
      } catch (error) {
      ...
      <Field name="id" label="ID" defaultValue={initial?.id} disabled={mode === "edit"} />
      <SelectField name="productId" label="Producto" options={products.map((product) => product.id)} defaultValue={initial?.productId} />
      <SelectField name="presentationType" label="Tipo" options={["barril20L", "barril30L", "barril50L", "growler1L", "growler2L", "porron500ml"]} defaultValue={initial?.presentationType} />
      <Field name="label" label="Etiqueta" defaultValue={initial?.label} />
      <Field name="volumeLiters" label="Volumen L" type="number" step="0.1" defaultValue={initial?.volumeLiters ?? ""} />
      <Field name="unitPrice" label="Precio" type="number" defaultValue={initial?.unitPrice ?? 0} />
      <SelectField name="category" label="Categoría" options={["barril", "growler", "porrón", "pack"]} defaultValue={initial?.category} />
      <Field name="description" label="Descripción" defaultValue={initial?.description ?? ""} required={false} />
      <Field name="sortOrder" label="Orden" type="number" defaultValue={initial?.sortOrder ?? 0} />
    </AdminForm>
  );
}
...
function WhatsAppForm({ disabled, initial, onCancel, onSubmit }: { disabled: boolean; initial?: WhatsAppFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar WhatsApp ${editId}` : "Crear WhatsApp"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear canal"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parseWhatsAppUpdateForm(input) : parseWhatsAppForm(input);
        setError("");
      } catch (error) {
      ...
      <SelectField name="purpose" label="Uso" options={["orders", "contact", "orders_and_contact"]} defaultValue={initial?.purpose} />
      <Field name="sortOrder" label="Orden" type="number" defaultValue={initial?.sortOrder ?? 0} />
    </AdminForm>
  );
}

function DeliveryForm({ disabled, initial, onCancel, onSubmit }: { disabled: boolean; initial?: DeliveryFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar entrega ${editId}` : "Crear entrega"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear entrega"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parseDeliveryUpdateForm(input) : parseDeliveryForm(input);
        setError("");
      } catch (error) {
      ...
      <SelectField name="requiresAddress" label="Requiere dirección" options={["true", "false"]} defaultValue={String(initial?.requiresAddress ?? true)} />
      <Field name="sortOrder" label="Orden" type="number" defaultValue={initial?.sortOrder ?? 0} />
    </AdminForm>
  );
}

function ExtraForm({ disabled, initial, onCancel, onSubmit }: { disabled: boolean; initial?: ExtraFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar extra ${editId}` : "Crear extra"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear extra"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parseExtraUpdateForm(input) : parseExtraForm(input);
        setError("");
      } catch (error) {
...
function PromotionForm({ disabled, initial, onCancel, onSubmit }: { disabled: boolean; initial?: PromotionFormValues; onCancel?: () => void; onSubmit: (input: Record<string, unknown>, mode: "create" | "edit") => Promise<boolean> }) {
  const [error, setError] = useState("");
  const mode = initial ? "edit" : "create";
  const editId = initial?.id;
  return (
    <AdminForm title={editId ? `Editar promoción ${editId}` : "Crear promoción"} error={error} disabled={disabled} submitLabel={mode === "edit" ? "Guardar cambios" : "Crear promoción"} onCancel={onCancel} onSubmit={async (event) => {
      event.preventDefault();
      const input = Object.fromEntries(new FormData(event.currentTarget));
      try {
        mode === "edit" ? parsePromotionUpdateForm(input) : parsePromotionForm(input);
        setError("");
      } catch (error) {
      ...
      <SelectField name="type" label="Tipo" options={["percentage", "fixed"]} defaultValue={initial?.type} />
      <Field name="value" label="Valor" type="number" step="0.01" defaultValue={initial?.value ?? 0} />
      <Field name="startDate" label="Desde" type="date" defaultValue={initial?.startDate ?? ""} required={false} />
      <Field name="endDate" label="Hasta" type="date" defaultValue={initial?.endDate ?? ""} required={false} />
    </AdminForm>
  );
}
  ...
  return (
    <form className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-card border border-white/10 rounded-lg p-4" onSubmit={onSubmit}>
      ...
      <div className="md:col-span-4 flex flex-wrap gap-2">
        ...
        {onCancel && <Button type="button" variant="outline" disabled={disabled} onClick={onCancel}><X className="w-4 h-4 mr-2" /> Cancelar</Button>}
      </div>
    </form>
  );
}

function ToggleTable<T extends { id: string; label: string; price: number; active: boolean }>({ rows, disabled, onEdit, onArchive, onRestore }: { rows: T[]; disabled: boolean; onEdit: (row: T) => void; onArchive: (row: T) => void; onRestore: (row: T) => void }) {
  return (
    <AdminTable
      rows={rows}
      ...
      render={(row) => [
        ...
        `$${row.price}`,
        row.active ? "sí" : "no",
        <RowActions key="actions" archived={!row.active} disabled={disabled} onEdit={() => onEdit(row)} onArchive={() => onArchive(row)} onRestore={() => onRestore(row)} />,
      ]}
    />
  );
}

function ListToolbar({ status, onStatusChange }: { status: StatusFilter; onStatusChange: (status: StatusFilter) => void }) {
  return (
...
function StatusFilterControl({ value, onChange }: { value: StatusFilter; onChange: (status: StatusFilter) => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      {(["all", "active", "archived"] as const).map((status) => (
        <Button key={status} type="button" size="sm" variant={value === status ? "default" : "outline"} onClick={() => onChange(status)}>
...
function Field({ name, label, className, required = true, ...props }: { name: string; label: string; className?: string } & InputHTMLAttributes<HTMLInputElement>) {
  return (
...
function SelectField({ name, label, options, defaultValue }: { name: string; label: string; options: string[]; defaultValue?: string }) {
  return (
    <div>
      <Label htmlFor={name}>{label}</Label>
      <select id={name} name={name} required defaultValue={defaultValue} className="h-9 w-full rounded-md border border-white/10 bg-black/40 px-3 text-sm text-white">
